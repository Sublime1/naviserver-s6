...
:END:
 ...
 |-------------------+---------+----------------------------------------|
 | NS_VERSION        | 4.99.21 | Define version for Naviserver          |
 |-------------------+---------+----------------------------------------|
 | NS_MODULE_VERSION | 4.99.21 | Define version for Naviserver  modules |
 |-------------------+---------+----------------------------------------|
...
#+begin_src sh :shebang ""
  # * From
  FROM oupfiz5/ubuntu-s6:20.04-3.0.0.0-1
...
#+begin_src sh :shebang ""
  ...
  # * Build Naviserver
  FROM oupfiz5/tcl-build:22.01 as build
  ...
  RUN export LANG="en_US.UTF-8" \
      ...
      && apt-get -y install libssl-dev locales libpq-dev postgresql-client wget \
...
** hook/build-msteps.sh :code:
:PROPERTIES:
:CUSTOM_ID: hook-build-msteps-sh
:END:
#+begin_src sh
  set -a; source ../VERSION ; set +a;
...
:END:
#+begin_src sh :dir ../../naviserver-s6/hook/
  set -a; source ../VERSION ; set +a;
