...
:END:
 ...
 |-------------------+---------+----------------------------------------|
 | NS_VERSION        | 4.99.23 | Define version for Naviserver-S6          |
 |-------------------+---------+----------------------------------------|
 | NS_MODULE_VERSION | 4.99.23 | Define version for Naviserver-S6  modules |
 |-------------------+---------+----------------------------------------|
...
#+begin_src sh :shebang ""
  ...
  UBUNTU_IMAGE_TAG="${UBUNTU_IMAGE_TAG:-20.04-3.0.0.2-2}"
#+end_src
  ...
  jobs:
    build:
      ...
      steps:
        ...
        - name: s6v2. Naviserver. Build and push image.
          run: |
            echo "::group::Prepare environment"
            set -a; source ${GITHUB_WORKSPACE}/src/VERSIONS ; set +a;
            export UBUNTU_IMAGE_TAG='20.04-2.2.0.3'
            export IMAGE_TAG="${UBUNTU_IMAGE_TAG}-${NS_IMAGE_TAG}"
            cd ${GITHUB_WORKSPACE}/src/hook
            echo "::endgroup::"
            echo "::group::Build image"
  ...
  jobs:
    checks:
      ...
      steps:
        ...
        - name: s6v2. Naviserver. Build. Check image and container.
          run: |
            echo "::group::Build image"
            set -a; source ${GITHUB_WORKSPACE}/src/VERSIONS ; set +a;
            export UBUNTU_IMAGE_TAG='20.04-2.2.0.3'
