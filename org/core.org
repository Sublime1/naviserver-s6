...
:CUSTOM_ID: about
:END:
This is [[https://wiki.tcl-lang.org/page/NaviServer][NaviServer]] on [[https://hub.docker.com/_/ubuntu][ubuntu base docker image]] (version 22.04) using [[https://github.com/just-containers/s6-overlay][s6-overlay]].  To install the NaviServer I used code from  [[https://github.com/gustafn/install-ns][Gustaf Neumann]] script [[https://github.com/gustafn/install-ns/blob/master/install-oacs.sh][install-ns.sh]].   The base image is [[https://hub.docker.com/r/oupfiz5/ubuntu-s6][oupfiz5/ubuntu-s6]].
...
CONTAINER_NAME="${CONTAINER_NAME:-naviserver-s6}"
NS_IMAGE_TAG="${NS_IMAGE_TAG:-4.99.24}"
IMAGE_NAME="${IMAGE_NAME:-naviserver-s6}"
...
ARG \
    ...
    RL_JSON_VERSION \
    TCLLIB_VERSION \
    TCL_BUILD_IMAGE_NAME \
    TCL_BUILD_IMAGE_REPOSITORY \
    TCL_BUILD_IMAGE_TAG \
    TCL_VERSION \
    TDOM_VERSION \
    THREAD_VERSION \
...
LABEL \
    ...
    custom.package.base_image="${UBUNTU_IMAGE_REPOSITORY}/${UBUNTU_IMAGE_NAME}:${UBUNTU_IMAGE_TAG}"
